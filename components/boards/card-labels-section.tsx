...
export function CardLabelsSection({
  ...
  function toggleAttached(labelId: string, isAttached: boolean) {
    ...
    run(() =>
      isAttached ? removeCardLabelAction(formData) : addCardLabelAction(formData),
    );
  }
    ...
    <section className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-xs text-muted-foreground">
          {canEdit ? "Manage labels" : "Visible to all members"}
        </span>
      </div>
      ...
      {canEdit ? (
        <div className="space-y-2 rounded-lg border bg-background p-3">
          <h4 className="text-sm font-semibold">Board labels</h4>
                ...
                editingId === label.id ? (
                  ...
                  >
                    <button
                      type="button"
                      className={cn(
                        "flex flex-1 items-center gap-2 rounded px-1 text-left text-sm disabled:opacity-50",
