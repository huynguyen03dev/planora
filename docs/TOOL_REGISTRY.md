...
## Planora Equipped Tools

The real tools available in this project, by workflow capability. Run these
`tool register` commands once to make the registry reflect reality, then
`scripts/bin/harness-cli tool check` to record presence.

| Capability | Tool | Kind | Command | Use in workflow |
| --- | --- | --- | --- | --- |
| `coverage` | vitest | cli | `npx vitest run` | Unit/integration proof (validation phase) |
| `lint` | eslint | cli | `npm run lint` | Static checks before closing a task |
| `db-migrate` | prisma | cli | `npx prisma migrate dev` | Data-model changes (high-risk gate) |
| `browser-testing` | chrome-devtools | mcp | `mcp:chrome-devtools` | Manual/E2E verification of UI flows |

```bash
scripts/bin/harness-cli tool register --name vitest --kind cli \
  --capability coverage --command "npx vitest run" --responsibility Verification \
  --description "Vitest unit/integration test runner (npm test)"

scripts/bin/harness-cli tool register --name eslint --kind cli \
  --capability lint --command "npm run lint" --responsibility Verification \
  --description "ESLint (eslint-config-next) static analysis check"

scripts/bin/harness-cli tool register --name prisma --kind cli \
  --capability db-migrate --command "npx prisma migrate dev" --responsibility Verification \
  --description "Prisma migrations and client generation for PostgreSQL"

scripts/bin/harness-cli tool register --name chrome-devtools --kind mcp \
  --capability browser-testing --scan ".mcp.json" --command "mcp:chrome-devtools" \
  --responsibility Verification --description "Chrome DevTools MCP for real-browser UI testing"
```

Notes specific to Planora:

- There is **no E2E or CI test gate** yet. `coverage` via vitest is the only
  automated proof; treat `browser-testing` as a manual capability until
  Playwright + a CI step exist.
- `db-migrate` work is a high-risk gate (see `docs/FEATURE_INTAKE.md`): schema
  changes require a migration, `npx prisma generate`, and a decision record.
